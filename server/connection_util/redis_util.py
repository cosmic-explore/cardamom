...
def match_from_json(json_str):
    ...
    match.board = Board(
        simple_dict["board"]["size_x"],
        simple_dict["board"]["size_y"],
        match=match,
        id=simple_dict["board"]["id"],
    )
    match.history = simple_dict["history"]
    match.creature_states = [
        CreatureState.from_dict(cs, match=match)
        for cs in simple_dict["creature_states"]
    ]
    return match
